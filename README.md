...
Веса такой сети, 209 чисел, это и есть ген трассы. Такие гены можно скрещивать и мутировать, чем и занимается отбор: 200 вариантов, 15 поколений, награда за разнообразие формы и штраф за непроходимые повороты. Полторы секунды на трассу.
...
203 теста, полторы минуты. Покрыто всё, что проверяется без окна: геометрия трассы, поле расстояний, датчики, физика, чекпоинты, приспособленность, генетический алгоритм, логика виджетов, синтез звука, сохранения.
...
Обучить настоящую генеративную модель для трасс и добавить переключатель между ней и текущим CPPN. У нас каждая трасса это 360 радиусов, то есть вектор фиксированной длины, и на таких данных VAE обучается легко. Датасет можно собрать своим же генератором с отбором либо взять геометрию реальных трасс.

Подробности в плане, задача 19.
