...
- `AiCar-<версия>-portable.zip` (30 МБ): распаковать и запустить `AiCar.exe`. Сохранения пишутся в папку рядом, на компьютере ничего не остаётся
- `AiCar-<версия>-setup.exe` (20 МБ): установщик с ярлыками и деинсталлятором. Прав администратора не просит, ставится в профиль пользователя
...
285 тестов, две минуты. Покрыто всё, что проверяется без окна: геометрия трассы, поле расстояний, датчики, физика, чекпоинты, приспособленность, генетический алгоритм, логика виджетов, синтез звука, сохранения. Одно исключение - тест открывает настоящее окно дважды: с 1.1.0 окно игры открывается там, где его закрыли (`window.json` в папке данных), и проверить это можно только у самой Windows.
...
98 свойств, которые обязаны выполняться при любых входных данных, каждое прогоняется сотнями раз на случайных входах: 8851 проверка на круг. Десять кругов идут около тринадцати минут, один - минуту с небольшим; на сервере сборки после каждого пуша прогоняется один круг вместе с тестами. Три из них гоняют настоящую игру сотни кадров подряд, случайно нажимая на всё подряд, и сверяют инварианты после каждого кадра. Тест стережёт придуманный автором сценарий, свойство стережёт договор целиком. Устройство разобрано в [docs/modules/stress.md](docs/modules/stress.md), находки в отчёте о проверке на баги.

Собранная версия проверяется отдельно, ключом `--selftest`: игра прогоняет полный раунд без окна и пишет отчёт в файл. Результат обязан совпасть с запуском из исходников до единицы. Сохранения игрока при этом не трогаются: запись уводится во временную папку.
...
`tools/make_previews.py` запускает игру без окна - тем же способом, что и её собственная самопроверка: SDL с драйвером dummy рисует в поверхность в памяти, кадр берётся прямо с неё.

```bash
python tools\make_previews.py
```

Снимок экрана тут не годится дважды. Окно может оказаться позади других, и в кадр попадёт чужое содержимое. А главное - нужный момент руками не поймать: кадр «часть поколения уже лежит серыми в одном повороте» живёт доли секунды. Скрипт ищет его по состоянию гонки, а не по номеру кадра, и с постоянным зерном повторяется от прогона к прогону.
