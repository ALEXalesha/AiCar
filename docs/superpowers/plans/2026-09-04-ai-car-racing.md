...
## Task 19: обученная модель генерации трасс (после основного проекта)

Отдельная задача, обсуждена 2026-09-04. Сейчас трассы рисует CPPN со случайными весами, обучения там нет. Хочется попробовать настоящую обученную модель.

Ключ к простоте: у нас трасса это `r(θ)` по 360 углам, то есть вектор фиксированной длины 360. Не картинка, не граф, не последовательность переменной длины. На таких данных обучается что угодно.

Откуда взять датасет, два пути:

- сгенерировать своим же CPPN несколько тысяч трасс, отсеять брак через `walls_are_sane` и отобрать по `track_fitness`. Модель научится воспроизводить распределение хороших трасс и генерировать их мгновенно, без 1.5 секунд эволюции
- взять геометрию реальных трасс Формулы-1, привести к виду `r(θ)` относительно центра масс, дополнить поворотами и отражениями

Что обучать: VAE на 360 радиусах, скрытое пространство размерности 8-16. Плюс VAE в том, что скрытое пространство непрерывно, и по нему можно ходить, получая плавные переходы между трассами. Отдельный интерес: посмотреть, что закодируется в каждой координате скрытого вектора.

Понадобится PyTorch, его сейчас в проекте нет.

Как сравнивать с текущим подходом: время генерации, доля брака, разнообразие кривизны, и субъективно на глаз.

В панели появляется переключатель генератора: `CPPN + эволюция` или `обученная модель`. Оба пути должны возвращать один и тот же объект `Track`, чтобы всё остальное в игре не заметило разницы. Практически это значит, что новый генератор выдаёт 360 радиусов, а сборка центральной линии, стен и чекпоинтов остаётся общей.

Переключатель имеет смысл и после сравнения: он позволяет прямо в игре увидеть, чем отличаются трассы двух генераторов, и это интереснее любых цифр.

Если файла обученной модели нет, пункт `обученная модель` в переключателе показывается неактивным, а не роняет игру.
